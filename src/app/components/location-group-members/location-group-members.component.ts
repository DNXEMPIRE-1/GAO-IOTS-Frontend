...
@Component({
  selector: 'app-location-group-members',
  templateUrl: './location-group-members.component.html',
  styleUrls: ['./location-group-members.component.css']
})
export class LocationGroupMembersComponent implements OnInit {
          ...
          this.dashboardService.getLocation(node.location.parentId).subscribe(
            responseData => {
              ...
              this.imgDialogRef = this.dialog.open(LocationImageDialogComponent , {
                data: { location: node.location, parentImage: parent.image}
              });
          }
        )
      }
  ...
  drop(event: Event) {
    ...
    {
      const result = this.groupMembers.find(s => s.id === this.draggingNode.location.id);
      if (result == null) // make sure no duplication
          this.groupMembers.push(this.draggingNode.location);
    }
    this.draggingNode = null!;
  }
  drag(node: LocationNode) {
  ...
  onRemove(location: RfidLocation)
  {
    ...
    if (objIndex > -1) {
      ...
      this.removeLocationGroup(location);
    }
  }
  removeLocationGroup(location: RfidLocation)
  {
    this.dashboardService.removeLocationGroup(location.id).subscribe(
      responseData => {
